...
public class ModelClassesController : ControllerBase
{
  ...
  // GET: api/ModelClasses/tentant/{tenantId}
  [HttpGet("tenant/{tenantId}")]
  public async Task<ActionResult<IEnumerable<ModelClass>>> GetModelClasses(string tenantId)
  {
    if (_context.ModelClasses == null) return NotFound();
    return await _context.ModelClasses
        .Where(modelClass => modelClass.TenantId == tenantId)
  ...
  public async Task<ActionResult<ModelClass>> GetModelClass(int id)
  {
    ...
    if (modelClass == null) return NotFound();
  ...
  {
    if (id != modelClass.ModelClassId) return BadRequest();
  ...
  {
    if (_context.ModelClasses == null) return Problem("Entity set 'EnpDBContext.ModelClasses'  is null.");

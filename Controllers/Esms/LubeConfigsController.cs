...
public class LubeConfigsController : BaeApiController<LubeConfigsController>
{
  ...
  public async Task<IEnumerable<LubeConfig>> Get()
  {
    return await _context.LubeConfigs
        ...
        .Include(con => con.ModelNavigation)
        .ToListAsync();
  }
  ...
  public async Task<IActionResult> GetById(int id)
  {
    ...
    if (lubeConfig == null) return NotFound();
  ...
  {
    var lubeConfig = _mapper.Map<LubeConfig>(lubeConfigPostDto);
  ...
  {
    if (id != lubeConfig.Id) return BadRequest();

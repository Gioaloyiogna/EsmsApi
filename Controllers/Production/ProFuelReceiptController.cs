...
namespace ServiceManagerApi.Controllers.Production
{
    ...
    public class ProFuelReceiptController : BaeApiController<ProFuelReceiptController>
    {
        ...
        // POST: api/ProFuelReceipt
        // To protect from overposting attacks, see https://go.microsoft.com/fwlink/?linkid=2123754
        [HttpPost]
        [ProducesResponseType(typeof(ProFuelIntake), StatusCodes.Status201Created)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<IActionResult> Create(IEnumerable<ProFuelReceiptPostDto> proFuelReceiptPostDto)
        {
            try
            {
                var proFuelIntake = _mapper.Map<IEnumerable<ProFuelIntake>>(proFuelReceiptPostDto);
        ...
        private bool IsProFuelReceiptPostDtoExist(ProFuelReceiptPostDto proFuelReceiptPostDto)
        {
            ...
            return _context.ProFuelIntakes.Any(e => 
                e.PumpId == proFuelReceiptPostDto.PumpId &&
                e.TenantId == proFuelReceiptPostDto.TenantId &&
                e.TransactionType == proFuelReceiptPostDto.TransactionType &&
                e.IntakeDate == proFuelReceiptPostDto.IntakeDate
            );
        }
    }
}
